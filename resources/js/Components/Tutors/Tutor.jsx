    ...
    return (
        <Link className="w-[22rem] rounded-xl border-neutral/70 shadow p-4" href={route('tutor', btoa(tutor.id))}>
            <div className="flex items-center mb-2">
                {tutor?.profile?.mediaLink ? <img src={tutor?.profile?.mediaLink} className="w-[64px] h-[64px] rounded-full mr-2" /> : <img src={avatarImage(tutor.name, 'random', 64)} alt={tutor.name} className="rounded-full mr-2" />}
                <div className="grow">
